...
import {
  ...
  updateBoatTable,
  deleteBoatTable,
  fetchBoatByGroupcationId,
  addBoatAttachmentsTable,
  deleteBoatAttachment,
...
const boatSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      // ADD NEW BOAT
      .addCase(addBoatTable.fulfilled, (state, action: PayloadAction<Boat>) => {
      ...
      .addCase(
        ...
        (
          ...
          // Find the boat
          const boat = state.boats.find((t) => Number(t.id) === Number(boatId));
          if (boat) {
      ...
      .addCase(
        ...
        (
          ...
          const boat = state.boats.find((t) => String(t.id) === String(boatId));
