...
import { fetchCelebrationTable, addCelebrationTable, updateCelebrationTable, deleteCelebrationTable, fetchCelebrationByGroupcationId, addCelebrationAttachmentsTable, deleteCelebrationAttachment, addCelebrationTravelersTable, deleteCelebrationTraveler } from "../thunk/celebrationThunk";
		...
		extraReducers: (builder) => {
			builder
				...
				.addCase(
					...
					(state, action: PayloadAction<CelebrationAttachments[]>) => {
						action.payload.forEach((attachment) => {
							...
							if (celebration) {
								celebration.attachments = [...(celebration.attachments || []), attachment];
							}
						});
					}
				)
					...
					(
						...
						if (celebration) {
							...
							celebration.attachments =
								celebration.attachments?.filter((att) => att.id !== attachmentId) || [];
						}
					}
				)
				...
				.addCase(
					...
					(
						state,
						action: PayloadAction<{ travelers: Traveler[]; celebrationId: string }>
					) => {
						...
						if (celebration) {
							...
							celebration.travelers = [...(celebration.travelers || []), ...travelers];
						}
					}
				)
					...
					(
						...
						if (celebration) {
							...
							celebration.travelers =
								celebration.travelers?.filter((att) => att.value !== travelerId) || [];
						}
					}
				);
		}
