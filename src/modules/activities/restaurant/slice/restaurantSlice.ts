...
import { fetchRestaurantTable, addRestaurantTable, updateRestaurantTable, deleteRestaurantTable, fetchRestaurantByGroupcationId, addRestaurantAttachmentsTable, deleteRestaurantAttachment, addRestaurantTravelersTable, deleteRestaurantTraveler } from "../thunk/restaurantThunks";
				...
				.addCase(
					...
					(state, action: PayloadAction<RestaurantAttachments[]>) => {
						action.payload.forEach((attachment) => {
							...
							if (restaurant) {
								restaurant.attachments = [...(restaurant.attachments || []), attachment];
							}
						});
					}
				)
					...
					(
						...
						if (restaurant) {
							...
							restaurant.attachments =
								restaurant.attachments?.filter((att) => att.id !== attachmentId) || [];
						}
					}
				)
				...
				.addCase(
					...
					(
						...
						if (restaurant) {
							...
							restaurant.travelers = [...(restaurant.travelers || []), ...travelers];
						}
					}
				)
